...
export function DetailModal({ artwork, onClose, isFavorited, onToggleFavorite, sessionId }: DetailModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end sm:items-center justify-center pb-16 sm:pb-0 bg-black/70 backdrop-blur-sm animate-fade-in"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        ref={modalRef}
        className="relative max-h-[95vh] sm:max-h-[90vh] w-full max-w-4xl flex flex-col overflow-hidden rounded-t-3xl sm:rounded-3xl backdrop-blur-2xl bg-surface-card/95 border border-glass-border shadow-2xl"
      ...
      >
        ...
        {/* Curator trigger — pinned at bottom, always visible */}
        {!sheet.isOpen && (
          <div className="flex-shrink-0 px-5 sm:px-6 pb-5 sm:pb-6 pt-3 border-t border-glass-border">
            <button
              onClick={sheet.open}
              className="w-full rounded-2xl bg-accent/15 border border-accent/25 px-4 py-3.5 text-sm font-medium text-accent hover:bg-accent/25 transition-colors flex items-center justify-center gap-2"
